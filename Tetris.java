...
public class Tetris implements World {
  ...
  public Tetris(int width, int height) {
    ...
    this.board = new Tetromino[BOARD_HEIGHT][BOARD_WIDTH];
    for (int i = 0; i < BOARD_HEIGHT; i++) {
      for (int j =0; j < BOARD_WIDTH; j++) {
        this.board[i][j] = Tetromino.NillShape;
      }
    }
  ...
  public boolean colliding() {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (board[y][x] != Tetromino.NillShape) {
        return true;
      }
    }
    return false;
  ...
  public void draw(Graphics g) {
    ...
    for (int i = 0; i < BOARD_HEIGHT; i++) {
      for (int j = 0; j < BOARD_WIDTH; j++) {
        if (this.board[i][j] != Tetromino.NillShape) {
          g2.setColor(Color.BLACK);
   ...
   */
  public boolean isFullLine(Tetromino[] row) {
    
    for (Tetromino t : row) {
      if (t == Tetromino.NillShape) {
        return false;
      }
    }
    return true;
